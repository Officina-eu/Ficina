//! `identityctl` — the non-public provisioning CLI for `ficina-identity`.
...
//! surface: the admin password arrives from `FICINA_ADMIN_PASSWORD` or
//! stdin, never a command-line argument (which would leak to the process
//! table).
//!
//! Env: `DATABASE_URL`, `FICINA_IDENTITY_ISSUER`, and optionally
//! `FICINA_IDENTITY_BLOB_DIR` (default `./blobs`; unused by identity ops but
...
/// Reads the admin password from `FICINA_ADMIN_PASSWORD`, else prompts on
/// stdin. Never taken from argv.
fn read_password() -> Result<String, String> {
    if let Ok(p) = std::env::var("FICINA_ADMIN_PASSWORD") {
        return Ok(p);
    }
...
fn fail(err: ficina_identity::IdentityError) -> String {
...
fn usage() -> ExitCode {
    eprintln!(
        "identityctl — Ficina identity provisioning\n\
         \n\
         commands:\n\
         ...
         \n\
         password: set FICINA_ADMIN_PASSWORD or type it when prompted (never argv)."
    );
    ExitCode::FAILURE
}
