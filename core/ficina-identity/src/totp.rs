...
mod tests {
    ...
    fn provisioning_uri_is_well_formed() {
        let uri = provisioning_uri("id.ficina.test", "alice@ficina.test", "JBSWY3DPEHPK3PXP");
        assert!(uri.starts_with("otpauth://totp/id.ficina.test:alice%40ficina.test?"));
        assert!(uri.contains("secret=JBSWY3DPEHPK3PXP"));
        assert!(uri.contains("issuer=id.ficina.test"));
