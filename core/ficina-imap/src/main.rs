...
//!
//! Beyond the `FICINA_IMAP_*`/`FICINA_POP3_*` listener config
//! ([`Config::from_env`]), this reads:
//! - `DATABASE_URL` — the Postgres system of record (required),
//! - `FICINA_BLOB_DIR` — the on-disk blob backend, shared with the SMTP
//!   service so IMAP serves the bodies SMTP delivered (required),
//! - `FICINA_IDENTITY_ISSUER` — the OIDC issuer, for the credential
...
async fn run(config: Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!(
        ...
        "ficina-imap starting"
    );
    ficina_imap::serve(config, store, identity).await?;
    Ok(())
}
