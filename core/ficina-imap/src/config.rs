//! Server configuration from the environment, mirroring `ficina-smtp`'s
//! `FICINA_*` conventions. Listeners are off unless their address is set.
...
impl Config {
    /// Builds a configuration from `FICINA_IMAP_*`/`FICINA_POP3_*`
    ...
    pub fn from_env() -> crate::Result<Self> {
        let d = Self::default();
        Ok(Self {
            imap_addr: parse_addr("FICINA_IMAP_ADDR")?,
            imaps_addr: parse_addr("FICINA_IMAPS_ADDR")?,
            pop3s_addr: parse_addr("FICINA_POP3S_ADDR")?,
            hostname: std::env::var("FICINA_IMAP_HOSTNAME").unwrap_or(d.hostname),
            tls_cert: std::env::var("FICINA_IMAP_TLS_CERT")
                .ok()
                .map(PathBuf::from),
            tls_key: std::env::var("FICINA_IMAP_TLS_KEY").ok().map(PathBuf::from),
            allow_self_signed: std::env::var("FICINA_IMAP_ALLOW_SELF_SIGNED")
                .map(|v| v == "true" || v == "1")
                .unwrap_or(false),
