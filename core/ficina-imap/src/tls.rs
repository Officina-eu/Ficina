...
//!
//! Deliberately mirrors `ficina-smtp`'s `tls.rs` rather than sharing a
...
mod tests {
    ...
    fn self_signed_acceptor_builds() {
        let acceptor = build_acceptor(None, None, "imap.ficina.test", true).unwrap();
        let _clone = acceptor.clone();
    }

    #[test]
    fn self_signed_refused_when_not_permitted() {
        match build_acceptor(None, None, "imap.ficina.test", false) {
            Err(ImapError::Tls { .. }) => {}
