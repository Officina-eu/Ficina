...
async fn quota_get_reports_tenant_storage() {
    ...
    let (_s, body) = api(&h.app, &h.token, call(&acc, "Quota/get", json!({ "ids": null }))).await;
    assert_eq!(
        body["methodResponses"][0][1]["list"].as_array().unwrap().len(),
        0,
        "unlimited tenant reports no quota: {body}",
    );

    // Give the tenant a cap and store some bytes.
    h.store.set_tenant_quota(&h.tenant, Some(10_000_000)).await.unwrap();
    ...
    assert!(q["used"].as_i64().unwrap() > 0, "used bytes are accounted: {body}");

    // An unknown quota id is notFound.
    let (_s, body) = api(&h.app, &h.token, call(&acc, "Quota/get", json!({ "ids": ["nope"] }))).await;
    assert_eq!(body["methodResponses"][0][1]["notFound"][0], json!("nope"), "{body}");
    assert!(body["methodResponses"][0][1]["list"].as_array().unwrap().is_empty(), "{body}");
}
