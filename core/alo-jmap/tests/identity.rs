...
async fn identity_get_lists_send_addresses() {
    ...
    let (_s, body) = api(&h.app, &h.token, call(&acct, "Identity/get", json!({ "ids": null }))).await;
    assert_eq!(body["methodResponses"][0][0], json!("Identity/get"), "{body}");
    let list = body["methodResponses"][0][1]["list"].as_array().unwrap();
    // Canonical address + the alias, one identity each.
    assert!(list.len() >= 2, "expected canonical + alias identities: {body}");
    for ident in list {
        assert!(ident["id"].as_str().is_some(), "identity has an id: {body}");
        assert!(ident["email"].as_str().is_some(), "identity has an email: {body}");
        assert_eq!(ident["mayDelete"], json!(false), "provisioned identity: {body}");
    }
    ...
    let (_s, body) = api(
        &h.app,
        &h.token,
        call(&acct, "Identity/get", json!({ "ids": [first_id, "deadbeefdeadbeef"] })),
    )
    .await;
    let list = body["methodResponses"][0][1]["list"].as_array().unwrap();
    assert_eq!(list.len(), 1, "one identity for the known id: {body}");
    assert_eq!(list[0]["id"].as_str().unwrap(), first_id);
    assert_eq!(body["methodResponses"][0][1]["notFound"][0], json!("deadbeefdeadbeef"));
}
