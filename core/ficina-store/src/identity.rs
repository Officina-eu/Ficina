//! Identity persistence for `ficina-identity` (Law 3: kept out of
//! `store.rs`). This module is **storage only** — all cryptography
//! (argon2id password hashing/verification, token hashing, TOTP, JWT
//! signing) lives in `ficina-identity`. The store persists values already
...
pub struct CredentialRow {
    ...
    /// The owning user.
    pub user: UserId,
    /// The argon2id PHC hash string. Verified by `ficina-identity`.
...
impl TenantStore {
    ...
    /// login username. The hash is produced by `ficina-identity`; the store
    ...
    /// (the plaintext codes are shown once by `ficina-identity` and never
    /// stored).
    ///
    /// # Errors
    /// [`StoreError::NotFound`] if the user is not in this tenant.
    ...
    /// `ficina-identity` to constant-time compare against a presented code.
    ///
    /// # Errors
    /// [`StoreError::Db`] on a database failure.
