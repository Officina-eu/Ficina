...
use ficina_sieve::{Action, EvalContext, Limits, Message as SieveMsg};
...
impl AccountStore {
    ...
    pub async fn put_sieve_script(&self, name: &str, content: &str) -> Result<()> {
        ...
        if let Err(e) = ficina_sieve::compile(content, Limits::default()) {
    ...
    fn evaluate(
        ...
        let script = match ficina_sieve::compile(content, Limits::default()) {
            Ok(s) => s,
            Err(e) => {
        ...
        match ficina_sieve::evaluate(&script, &msg, &ctx) {
...
/// `ficina-imap`'s mapping (the store cannot depend on the imap crate).
