//! `Store`, `TenantStore`, and the door to accounts — isolation by
//! construction.
//!
//! `Store` does system operations only (tenants, migrations, interim
//! auth). A [`TenantStore`], obtained via [`Store::for_tenant`], narrows
//! access to one tenant and exposes only genuinely tenant-level
//! operations: user lookup, credential and account administration. All
//! **user-owned** data — messages, mailboxes, threads, keywords, the
//! per-account change log, and the blobs a user's mail references — is
//! reachable **only** through an [`AccountStore`](crate::AccountStore),
//! obtained via [`Store::for_account`]; every statement it issues carries
//! `tenant_id = $tenant AND user_id = $user` by construction, so a
//! cross-account access is unrepresentable in the API and returns
//! `NotFound` in the data.
...
impl Store {
    ...
    /// The door to tenant-level administration: a handle scoped to one
    /// tenant, exposing user lookup and credential/account admin. Pure —
    /// no I/O. Mail data is **not** reachable here; use [`Self::for_account`].
    ...
    /// `(tenant, user)`. Pure — no I/O; every operation it exposes carries
    /// both predicates by construction, so cross-account access is
    /// unrepresentable (see [`AccountStore`]).
    pub fn for_account(&self, tenant: TenantId, user: UserId) -> AccountStore {
        AccountStore {
            pool: self.pool.clone(),
...
/// A tenant-scoped handle for tenant-level administration only. Holds its
/// [`TenantId`] privately and bakes it into every statement. No method
/// accepts a tenant argument, and none reaches user-owned mail data —
/// that lives behind [`AccountStore`](crate::AccountStore).
#[derive(Clone)]
...
impl TenantStore {
    ...
    /// Confirms a user exists in this tenant. `NotFound` otherwise — so a
    /// stale reference cannot create an orphan row.
