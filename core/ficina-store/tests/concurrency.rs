...
async fn concurrent_add_to_mailbox_is_idempotent() {
    ...
    assert_eq!(
        ts.list_mailbox(&folder, ficina_store::Page::default())
            .await
            .unwrap()
            .len(),
        1
...
async fn set_seen_racing_membership_never_drifts_or_underflows() {
    ...
    for i in 0..48 {
        ...
        handles.push(tokio::spawn(async move {
            match i % 3 {
                0 => {
                    let _ = ts
                        .set_keyword(&message, ficina_store::SEEN, i % 2 == 0)
                        .await;
                }
                1 => {
    ...
    let seen = ts
        ...
        .await
        .unwrap()
        .contains(&ficina_store::SEEN.to_owned());
    for mb_id in [&inbox, &folder] {
        ...
        let present = ts
            .list_mailbox(mb_id, ficina_store::Page::default())
            .await
            .unwrap()
            .len() as i64;
