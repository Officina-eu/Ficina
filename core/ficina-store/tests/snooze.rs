...
async fn snooze_moves_out_of_inbox_and_the_sweeper_brings_it_back() {
    ...
    assert!(acc.mailboxes_of_message(&mid).await.unwrap().contains(&inbox));

    // --- snooze into the future: leaves the Inbox, lands in Snoozed ---
    let future = 4_102_444_800_i64; // 2100-01-01
    acc.snooze(std::slice::from_ref(&mid), &inbox, future).await.unwrap();
    ...
    acc.snooze(std::slice::from_ref(&mid), &inbox, 1_000_000_000_i64).await.unwrap(); // 2001
    ...
    assert_eq!(store.sweep_snoozes().await.unwrap(), 0, "wake time should be cleared");
