...
async fn check_dkim(resolver: &TokioResolver, domain: &str) -> Value {
    let selector = std::env::var("FICINA_SMTP_DKIM_SELECTOR")
        .ok()
        .filter(|s| !s.trim().is_empty());
    let dkim_domain = std::env::var("FICINA_SMTP_DKIM_DOMAIN")
        .ok()
        .filter(|s| !s.trim().is_empty())
    ...
    let Some(selector) = selector else {
        return item(
            ...
            "DKIM",
            Status::Warn,
            "DKIM is not configured on this server (set FICINA_SMTP_DKIM_* and publish the key)."
                .to_owned(),
        );
    };
