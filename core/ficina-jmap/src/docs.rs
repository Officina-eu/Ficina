//! Ficina Docs HTTP API (ADR 0015): tenant- and owner-scoped CRUD for
...
use ficina_store::{Document, DocumentSummary, StoreError};
...
pub async fn save(
    ...
    if body.len() > MAX_DOC_BYTES {
        return Err(Problem::with(StatusCode::PAYLOAD_TOO_LARGE, "document too large"));
    }
    let request: Value = serde_json::from_slice(&body).map_err(|_| Problem::not_json())?;
    let title = clean_title(request.get("title").and_then(Value::as_str).unwrap_or(""));
    let blocks = request.get("blocks").cloned().unwrap_or_else(|| json!([]));
    if !blocks.is_array() {
        return Err(Problem::with(StatusCode::BAD_REQUEST, "blocks must be an array"));
    }
...
fn clean_title(raw: &str) -> String {
    let cleaned: String = raw.replace(['\r', '\n', '\t'], " ").trim().chars().take(MAX_TITLE).collect();
    if cleaned.is_empty() {
