...
fn submission_problem(err: &Value) -> Problem {
    let kind = err.get("type").and_then(Value::as_str).unwrap_or("invalidProperties");
...
pub async fn send_later(
    ...
    let send_at = body
        ...
        .ok_or_else(|| Problem::with(StatusCode::BAD_REQUEST, "sendAt is required and must be a near-future time"))?;
