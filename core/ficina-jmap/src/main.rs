...
//! endpoints are mounted by [`ficina_jmap::app`], so this one service is
//! both the native mail API and the identity provider.
//!
//! Environment:
//! - `DATABASE_URL` — the Postgres system of record (required),
//! - `FICINA_BLOB_DIR` — the on-disk blob backend, shared with the SMTP and
//!   IMAP services (required),
//! - `FICINA_IDENTITY_ISSUER` — the OIDC issuer URL, the public HTTPS origin
//!   this service is reached at, e.g. `https://mail.example` (required),
//! - `FICINA_JMAP_ADDR` — the internal bind address (default `0.0.0.0:8080`;
//!   TLS is terminated by the front proxy),
//! - `FICINA_JMAP_BASE_URL` — the external base URL for session resource
...
async fn run(addr: SocketAddr) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_url = std::env::var("FICINA_JMAP_BASE_URL")
        .ok()
        .filter(|v| !v.is_empty())
...
fn bind_addr() -> Result<SocketAddr, String> {
    let raw = std::env::var("FICINA_JMAP_ADDR").unwrap_or_else(|_| DEFAULT_ADDR.to_owned());
    raw.parse()
        .map_err(|e| format!("FICINA_JMAP_ADDR: invalid socket address {raw:?}: {e}"))
}
