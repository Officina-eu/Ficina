...
pub async fn download(
    ...
    // Account scope is structural: `blob`/`blob_bytes` on the account
    // store resolve only blobs a message of *this* account references, so
    // a foreign or unreferenced blob is `NotFound` — no `owns_*` gate to
    // forget.
    let meta = account.store.blob(&id).await.map_err(store_problem)?;
    let bytes = account.store.blob_bytes(&id).await.map_err(store_problem)?;
