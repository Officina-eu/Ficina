...
//! existing Sieve API. The delivery-time evaluator (`ficina-smtp` →
...
const MANAGED_SCRIPT: &str = "ficina-mail-rules";
    ...
    for rule in rules {
        ...
        for c in &rule.conditions {
            ...
            if v.is_empty() || v.len() > MAX_VALUE_LEN || has_control(v) {
                return Err(bad("a condition value is empty, too long, or has control characters"));
            }
        }
...
fn is_block_of(rule: &Rule, email: &str) -> bool {
    rule.conditions.len() == 1
        ...
        && rule.actions.iter().any(|a| matches!(a, Action::FileInto { mailbox } if mailbox == "Junk"))
}
...
fn block_rule(email: &str) -> Rule {
    Rule {
        ...
        actions: vec![Action::FileInto { mailbox: "Junk".to_owned() }],
        enabled: true,
    }
}
...
fn generate_script(
    ...
    let uses_fileinto = active
        .iter()
        .any(|r| r.actions.iter().any(|a| matches!(a, Action::FileInto { .. })));
    let uses_flags = active
        .iter()
        .any(|r| r.actions.iter().any(|a| matches!(a, Action::MarkRead | Action::Star)));
    if uses_fileinto {
...
mod tests {
    ...
    fn generates_compilable_script_for_each_action() {
        let rules = vec![
            rule(r#"{"conditions":[{"field":"from","value":"spam@x.com"}],"actions":[{"type":"fileInto","mailbox":"Junk"}],"match":"any"}"#),
            rule(r#"{"conditions":[{"field":"subject","op":"contains","value":"invoice"},{"field":"from","value":"billing@acme.eu"}],"actions":[{"type":"fileInto","mailbox":"Finance"},{"type":"markRead"},{"type":"star"}],"match":"all"}"#),
            rule(r#"{"conditions":[{"field":"from","value":"noise@list.eu"}],"actions":[{"type":"delete"}],"match":"any"}"#),
        ];
        let script = generate_script(&rules, false, "", "").unwrap_or_else(|| panic!("expected a script"));
        // It must compile in the real engine.
        ficina_sieve::compile(&script, ficina_sieve::Limits::default()).unwrap_or_else(|e| panic!("did not compile: {e}\n{script}"));
    ...
    fn combines_filters_with_vacation_and_compiles() {
        ...
        let script = generate_script(&rules, true, "Away", "Back Monday").unwrap_or_else(|| panic!("expected a script"));
        ficina_sieve::compile(&script, ficina_sieve::Limits::default()).unwrap_or_else(|e| panic!("did not compile: {e}\n{script}"));
