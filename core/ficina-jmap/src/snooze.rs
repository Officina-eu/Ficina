...
use ficina_store::{MailboxId, MessageId, StoreError};
...
pub async fn snooze(
    ...
    let ids: Vec<MessageId> = req
        ...
        .map(|a| a.iter().filter_map(Value::as_str).map(MessageId::new).collect())
        .unwrap_or_default();
    let (Some(mailbox), Some(until)) = (
        req.get("mailboxId").and_then(Value::as_str),
        req.get("until").and_then(Value::as_i64),
    ) else {
        return Err(Problem::with(StatusCode::BAD_REQUEST, "mailboxId and until required"));
    };
    if ids.is_empty() {
