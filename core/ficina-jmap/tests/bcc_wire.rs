...
//! submission method against a REAL Postgres store and a REAL ficina-smtp-client
...
use ficina_jmap::PushHub;
use ficina_jmap::mime::{Addr, Outgoing, build};
use ficina_jmap::state::{Account, AppState, Limits};
use ficina_store::{BlobStore, MAX_PAGE, Page, Store};
use serde_json::json;
...
fn database_url() -> String {
    std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgres://ficina:ficina@127.0.0.1:5455/ficina".to_owned())
}
...
async fn bcc_stripped_on_wire_delivered_by_envelope_kept_in_sent() {
    ...
    let draft = Outgoing {
        from: Addr { name: None, email: sender.clone() },
        to: vec![Addr { name: Some("Alice To".into()), email: to.into() }],
        cc: vec![Addr { name: Some("Carol Cc".into()), email: cc.into() }],
        bcc: vec![Addr { name: Some("Dave Bcc".into()), email: bcc.into() }],
    ...
    let drafts = acc.create_mailbox(None, "Drafts", Some("drafts")).await.unwrap();
    let _sent = acc.create_mailbox(None, "Sent", Some("sent")).await.unwrap();
    ...
    let resp = ficina_jmap::submission::set(&account, &args, &state)
        .await
        .expect("EmailSubmission/set returned a method-level error");
    assert!(resp["created"]["c1"].is_object(), "submission was not created: {resp}");
    ...
    assert!(!wire.contains(bcc), "the bcc address string appears in the wire bytes");

    assert!(rcpts.iter().any(|r| r.contains(bcc)), "Bcc recipient not in envelope RCPT TO");
    ...
    assert!(sent_bcc_line.contains(bcc), "Sent copy Bcc lacks the bcc address");
    ...
    assert!(member_of.iter().any(|m| m == &sent_id), "not filed into Sent");
    assert!(!member_of.iter().any(|m| m == &drafts), "still in Drafts after send");
