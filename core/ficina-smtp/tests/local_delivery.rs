...
use ficina_smtp::local_delivery::{DeliveryOutcome, LocalDelivery};
use ficina_smtp::server;
use ficina_smtp::spool::Spool;
use ficina_store::{BlobStore, MailboxId, Page, Store};
...
async fn make_user(store: &Store, tag: &str) -> (ficina_store::AccountStore, String, MailboxId) {
    let tenant = store.create_tenant(&format!("ld-{tag}")).await.unwrap();
    let ts = store.for_tenant(tenant.clone());
    let email = format!("{tag}-{tenant}@ficina.test");
...
async fn count(acc: &ficina_store::AccountStore, mb: &MailboxId) -> usize {
...
async fn message_over_smtp_lands_in_the_mailbox_and_unknown_rcpt_is_550() {
    ...
    let unknown = c.cmd("RCPT TO:<nobody@ficina.test>").await;
    assert!(
