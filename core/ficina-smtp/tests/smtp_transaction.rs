...
//!
//! Like `smtp_wire.rs`, tests run against `FICINA_SMTP_TEST_ADDR`
...
const TEST_HOSTNAME: &str = "mx.ficina.test";
const IO_TIMEOUT: Duration = Duration::from_secs(10);

fn external_addr() -> Option<SocketAddr> {
    std::env::var("FICINA_SMTP_TEST_ADDR")
        .ok()
        .map(|s| s.parse().expect("FICINA_SMTP_TEST_ADDR must be host:port"))
}
