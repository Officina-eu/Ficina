...
async fn starttls_then_auth_then_submit_end_to_end() {
    ...
    assert!(
        plain
            .cmd("MAIL FROM:<alice@ficina.test>")
            .await
            .starts_with("530 ")
    );
    ...
    assert!(
        sec.cmd("MAIL FROM:<alice@ficina.test>")
            .await
            .starts_with("530 ")
    );

    // Wrong password → 535.
    let bad = BASE64.encode("\u{0}alice@ficina.test\u{0}wrong");
    assert!(
    ...
    let good = BASE64.encode("\u{0}alice@ficina.test\u{0}s3cret");
    assert!(
    ...
    assert_eq!(envelope.mail_from.as_deref(), Some("alice@ficina.test"));
...
async fn auth_login_mechanism_works_over_tls() {
    ...
    let user_reply = sec.cmd(&BASE64.encode("alice@ficina.test")).await;
