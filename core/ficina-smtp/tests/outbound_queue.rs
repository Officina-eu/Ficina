...
use ficina_smtp::envelope::Envelope;
use ficina_smtp::queue::{Queue, QueuePolicy, Route};
use ficina_smtp::resolver::{MxResolve, ResolveFailure, ResolveFuture};
...
async fn queue_with(behaviour: Behaviour) -> (Arc<Spool>, Queue, tempfile::TempDir) {
    ...
    let queue = Queue::new(
        ...
        policy("mx.ficina.test", mock.addr),
    );
    (spool, queue, dir)
}
...
async fn transient_failure_defers_with_durable_state() {
    ...
    assert_eq!(report2, ficina_smtp::queue::PassReport::default());
}
...
async fn permanent_failure_bounces_with_dsn_from_null_sender() {
    ...
    assert!(body.contains("MAILER-DAEMON@mx.ficina.test"));
}
