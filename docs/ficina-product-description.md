...
*Working name: Ficina (from the Latin officina, "workshop" — the root of the word "office"). Alternative under consideration: Atelier, pending trademark search.*

---

## 1. What Ficina is

Ficina is a complete replacement for Microsoft 365, built and hosted in Europe. It combines mail, calendar, chat, video meetings, file storage, and document editing into one product, unified by an AI assistant that runs on European infrastructure and sees across everything — mail, conversations, meetings, and files — inside a single tenant.

It is offered two ways from the same codebase:

- **Ficina Cloud** — hosted by us on EU infrastructure. The customer changes DNS, migrates, and is done. Recurring subscription.
- **Ficina Self-Hosted** — the same product, deployed on the customer's own hardware or European VPS, for organizations that require full physical control. License and support revenue.
...
Ficina is built against each of those failures:

- **Better, not parity.** Ficina is the first mail system where the server itself is intelligent — semantic memory of every thread, automatic triage, drafted replies, cross-channel answers ("what did I miss this week?" drawn from mail, chat, and meetings together). Microsoft bolts Copilot onto 30-year-old plumbing at a premium; Ficina is built around the model, with data that never leaves the tenant.
...
- **Channel from day one.** European MSPs and resellers earn recurring margin on Ficina; the people who currently install Microsoft become the people who install Ficina.
...
| Module | What it does |
|---|---|
| **Ficina Mail** | Full business email: mailboxes, aliases, shared mailboxes, native distribution lists, server-side rules (Sieve), archiving, spam and phishing filtering. |
| **Ficina Agenda** | Calendars, shared calendars, free/busy, meeting invitations, room and resource booking, out-of-office. |
| **Ficina Chat** | Slack-model messaging: channel-first, real threads, reactions, powerful search, guest access. History is queryable knowledge, not a paywalled archive. |
| **Ficina Meet** | Video meetings integrated with Agenda and Chat; meeting links native to the calendar. |
| **Ficina Drive** | File storage and sharing with permissions, synced to desktop, serving Office-format files losslessly. |
| **Ficina Docs** | Documents, spreadsheets, and presentations edited in the browser, in Microsoft formats, embedded under Ficina's interface. |
| **Ficina AI** | The unifier: semantic search across all modules, inbox triage, thread and meeting summarization, drafted replies, attachment understanding (reads incoming .docx/.xlsx), an in-product "where did X go?" onboarding assistant, and an MCP server so customers' AI agents can work with their workspace. |
| **Ficina Migrate** | The M365 exit suite — see section 6. |
...
| Component | Role |
|---|---|
| `ficina-smtp` | Mail transfer and submission: receiving (port 25), client submission (587), queueing, routing, retries. |
| `ficina-auth-mail` | DKIM signing/verification, SPF, DMARC, ARC, TLS, MTA-STS/DANE. |
| `ficina-store` | The message and blob store: mailboxes, threads, flags, attachments, full-text indexing, on PostgreSQL + object storage. |
| `ficina-jmap` | JMAP API (RFC 8620/8621) — the native protocol for our own clients. |
| `ficina-imap` | IMAP/POP3 compatibility shim over the store for legacy clients. |
| `ficina-dav` | CalDAV/CardDAV server: calendars, contacts, invitations (iTIP/iMIP), free/busy. |
| `ficina-identity` | Users, groups, OIDC/SAML identity provider, LDAP sync, 2FA. Ficina can act as the company's SSO. |
| `ficina-ai` | Event-bus indexer, per-tenant semantic index, LLM orchestration, triage/summarization/drafting workers, MCP server. |
| `ficina-control` | Multi-tenant control plane: provisioning, quotas, billing hooks, monitoring. |
| `ficina-migrate` | The full M365 migration suite (Graph API based). |
...
| **Desktop & mobile shells** | PWA at launch; Tauri desktop app in phase two — the same TypeScript UI inside a thin Rust shell (tray, native notifications, autostart, deep links), followed by an offline-first layer: a local Rust mail cache syncing over JMAP, sharing types and client logic with `ficina-core`, so desktop users get instant search and mail that works on the train. Mobile apps reuse the same UI and sync engine. No third language ever enters the codebase — Rust below the waterline, TypeScript above it, on every platform. |

### Integrated open source (pinned upstream containers, never forked)

| Project | Role in Ficina | Language (irrelevant to users, relevant to ops) | Origin |
|---|---|---|---|
| **Synapse (Matrix)** | Chat engine behind Ficina Chat. Ficina ships its own UI; Matrix is the invisible protocol. Swappable later for a Rust homeserver. | Python | UK/EU community |
| **LiveKit** | WebRTC engine behind Ficina Meet, used via its components SDK under our UI. | Go | Open source |
| **Collabora Online** (or OnlyOffice) | Document/spreadsheet/presentation editing in Microsoft formats, embedded via WOPI and themed to Ficina. | C++ / JS (OnlyOffice: C++ / Node.js) | UK–EU / Latvia |
| **Rspamd** | Spam and phishing scoring at SMTP time in phase one; augmented and potentially replaced by `ficina-ai` scoring later. | C / Lua | EU community |
...
Engines are appliances; Ficina is the building. Every integrated engine is contacted exclusively through its public API — Matrix client/admin APIs and application services for Synapse, SDK/JWT/webhooks for LiveKit, WOPI for Collabora (which calls Ficina Drive's endpoints), S3 and admin API for Garage. Three consequences follow: our code is never derivative of theirs (AGPL obligations stay at "link upstream"); every engine is swappable when an upstream falters, as MinIO's 2026 archival demonstrated; and no API limits exist, because the engines run on our infrastructure — the only rate limits in the system are the per-tenant fair-use limits we impose at our own gateway, which double as pricing-tier boundaries. Engines run unmodified as version-pinned containers; any unavoidable patch goes in a public patches repo and is offered upstream. No engine rebuild is considered before Ficina has paying customers — after that it is a deliberate investment decision per component, not a reflex.

## 5. Architecture

Five layers:

1. **Clients** — Ficina web/PWA/desktop/mobile apps; Outlook and phones via compatibility adapters; any standard IMAP/DAV/Matrix client.
...
- **Identity** — imports users and groups from Entra ID; Ficina becomes the OIDC/SAML identity provider so every other SaaS login keeps working and the hidden Microsoft lock-in dies.
...
- **Calendar continuity** — recurring events with exceptions, room/resource mailboxes, and rewriting of Teams meeting links in future events to Ficina Meet links.
...
- **Revenue:** Ficina Cloud subscriptions (priced visibly below the customer's current M365 tier, AI included rather than a €30/user add-on), self-hosted licenses and support, and MSP/reseller margin — the channel that made Odoo win, applied to communication software.
...
| 1 — Mail core | 1–7 | SMTP, auth stack, store, JMAP, IMAP shim, Sieve, identity | Founder lives on Ficina mail daily |
...
| 4 — Migration suite | 12–16 | Full Ficina Migrate as described above | A non-Axon pilot migrated in one weekend by their own IT |
...
Exchange-compatibility adapters (ActiveSync, then MAPI) are the year-two battle: launch sells Ficina's own apps plus open protocols; native Outlook support lands after.
...
**Landscape:** the sovereign-mail field is crowded but split — old technology with Exchange compatibility (Grommunio, IceWarp, Zextras Carbonio), or modern technology without it (Stalwart); infrastructure vendors (Open-Xchange/Dovecot) serve telcos, not SMEs; hosted providers (Proton, Infomaniak, mailbox.org) offer no self-hosting or suite depth; Nextcloud has the sovereignty brand but weak mail. Nobody is AI-native: AI is at best a webmail bolt-on, never a tenant-local layer across mail, chat, meetings, and files. Ficina's square — modern core + suite completeness + AI-native, on EU terms — is unoccupied because it is the hardest to reach; the corresponding competitive risk is Stalwart adding Exchange adapters or an incumbent bolting on credible AI before launch.
...
The AI layer is model-agnostic by design — an internal inference API (`ficina-ai`) routing to interchangeable backends, because the model landscape shifts faster than any other dependency:

- **Ficina Cloud default:** EU-hosted open-weight models (e.g. Mistral family) served from our own GPUs or an EU inference provider under DPA — customer data never reaches US endpoints.
...
Written down because scope creep killed most of our predecessors. Ficina will **not** build: office editors, a video/WebRTC stack, a Matrix homeserver, an object store, device management (MDM), a CRM/ERP (that is Odoo's territory — integrate, don't compete), a public federated social layer, or consumer/free-tier email. Entries here are revisited only with revenue on the table and a written case — the default answer stays no.

## 15. Open decisions

Tracked here until closed: final name (Ficina vs Atelier — EUIPO search pending); exact open-core boundary (which control-plane components stay proprietary); Collabora vs OnlyOffice as primary docs engine (Collabora leading); CLA tooling; hosting partner (Hetzner vs OVH vs Scaleway); pricing tiers; second developer hire (target: Phase 2 start).

## 16. Positioning in one line

Everyone else built a cheaper Exchange. **Ficina is the first workspace where the mail thinks — open, European, and complete enough to cancel Microsoft.**
